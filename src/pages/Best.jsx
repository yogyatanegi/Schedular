...
const Best = () => {
  ...
  return (
    <div className="p-4">
      ...
      {allResults.length > 0 && (
        <>
          <table className="mt-6 w-full border text-sm border-gray-300 dark:border-gray-600">
            <thead>
              <tr className="bg-gray-100 dark:bg-gray-800">
                <th className="border p-2 dark:border-gray-600">Algorithm</th>
                <th className="border p-2 dark:border-gray-600">Avg Turnaround Time</th>
                <th className="border p-2 dark:border-gray-600">Avg Waiting Time</th>
              </tr>
            </thead>
            <tbody>
              {allResults.map((algo, index) => (
                <tr key={index} className="bg-white dark:bg-gray-800">
                  <td className="border p-2 dark:border-gray-600">{algo.algorithm}</td>
                  <td className="border p-2 dark:border-gray-600">{algo.average_turnaround_time}</td>
                  <td className="border p-2 dark:border-gray-600">{algo.average_waiting_time}</td>
                </tr>
              ))}
            </tbody>
          </table>
          <div className="w-full bg-white dark:bg-gray-900 my-10 flex flex-row gap-5">
