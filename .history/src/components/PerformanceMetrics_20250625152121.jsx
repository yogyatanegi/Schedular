      ...
      <table className="w-full border-collapse border border-gray-300 dark:border-gray-600 text-center">
        <thead className={isNonPreemptive ? 'text-[0.82rem]' : 'text-[0.72rem]' }>
            <tr className="bg-gray-200 dark:bg-gray-700">
                ...
                <th className="border border-gray-300 dark:border-gray-600 px-4 py-[0.67rem]">Turnaround Time</th>
            </tr>
        </thead>
        ...
        {executed ? 
          <tbody>
            ...
            <tr key={process.processId} className="bg-white dark:bg-gray-900">
              ...
              {isNonPreemptive && (
                <td className="border border-gray-300 dark:border-gray-600 px-4 py-2">{process.startTime - process.arrivalTime}</td>
              )}
              {!isNonPreemptive && (
                <>
                  <td className="border border-gray-300 dark:border-gray-600 px-4 py-2">{(process.completionTime - process.arrivalTime) - process.burstTime}</td>
                  <td className="border border-gray-300 dark:border-gray-600 px-4 py-2">{process.startTime - process.arrivalTime}</td>
                </>
              )}
              <td className="border border-gray-300 dark:border-gray-600 px-4 py-2">{process.completionTime - process.arrivalTime}</td>
            </tr>
            ))} 
          </tbody>
         : ''
