const computeStatistics = (selectedAlgorithm, processes, setProcesses) => {
    ...
    for (let i = 0; i < processes.length; i++) {
        ...
        processes[i].waitingTime = isNonPreemptive ? processes[i].startTime - processes[i].arrivalTime : processes[i].completionTime - processes[i].burstTime - processes[i].arrivalTime;
