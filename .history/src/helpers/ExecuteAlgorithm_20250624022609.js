...
const executeSelectedAlgorithm = async (selectedAlgorithm, num_of_processes, processes, setProcesses, setUpdatedProcesses, setExecuted, setStatisticsData, setAnotherUpdatedProcesses,setCurrentProcess, getPaused, delayRef) => {
    ...
    switch (selectedAlgorithm) {
        case FCFS:
            await executeFCFS(processes, num_of_processes, setUpdatedProcesses, setCurrentProcess, getPaused, delayRef);
            setExecuted(true);
            ...
            break;  
        case SJF:
            await executeSJF(processes, num_of_processes, setUpdatedProcesses, setCurrentProcess, getPaused, delayRef);   
            setExecuted(true);
            data = computeStatistics(selectedAlgorithm, processes, setProcesses);
            setStatisticsData(prevData => [...prevData, { algorithm: "SJF", data }]);
            break;
        case PRIORITY:
            await executePriority(processes, num_of_processes, setUpdatedProcesses, setCurrentProcess, getPaused,delayRef);
            setExecuted(true);
            data = computeStatistics(selectedAlgorithm, processes, setProcesses);
